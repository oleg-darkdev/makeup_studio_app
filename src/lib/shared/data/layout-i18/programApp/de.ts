const de = {
	title: 'Kursprogramm',
	module: 'Modul',
	theory: 'Theoretischer Teil',
	practice: 'Praktischer Teil',
	courseName: 'EYE LINER',
	buttonBuy: 'Zugang kaufen',
	warning1: 'Der Live-Kurs ist nur verfügbar ',
	warning2: 'auf polnisch und russisch.',
	warning3: 'Für das Selbststudium ist der Kurs verfügbar auf ',
	warning4: 'russisch, englisch, polnisch, spanisch, französisch und deutsch.',
	courseData: [
		{
			title: 'Einführung in den Kurs',
			id: 1,
			avif: '/images/results/cropped/result_0.avif',
			link: '/', image: '/images/results/cropped/result_0.png',

			theory: [
				'Lernen wir uns kennen. Meine Geschichte. Meine Ästhetik.',
				'Aufbau des Augenlids. Verbotene Bereiche.',
				'Alterungsprozesse und deren Wechselwirkung.'
			],
			practice: []
		},
		{
			title: 'Skizze und Vorbereitung',
			id: 2,
			avif: '/images/results/cropped/result_2.avif',
			link: '/', image: '/images/results/cropped/result_2.png',

			theory: [
				'Grundsätze der Skizzenerstellung. Theoretischer Teil.',
				'Skizzenerstellung. Beispiel am Modell.',
				'Arten von Skizzen.',
				'Auswahl der Schattierungen je nach Augenform.'
			],
			practice: ['Skizzenerstellung. Beispiel am Modell.']
		},
		{
			title: 'Grundlagen der Technik und Schattierung',
			id: 3,
			avif: '/images/results/cropped/result_3.avif',
			link: '/', image: '/images/results/cropped/result_3.png',

			theory: [
				'Lange und kurze Bewegungen der Maschinen-Nadel.',
				'Grundlagen der Schattierung.',
				'Schattierung auf Latex: kurze und lange Schatten in Gruppen, schnell und langsam.',
				'Halbrundschatten.',
				'Halbrundschattierung: Übungen auf Latex, Schattierung in 4 Richtungen.'
			],
			practice: [
				'Halbrundschatten.',
				'Halbrundschattierung: Übungen auf Latex, Schattierung in 4 Richtungen.'
			]
		},
		{
			title: 'Verläufe, Farbdichte und Farbverarbeitung',
			id: 4,
			avif: '/images/results/cropped/result_5.avif',
			link: '/', image: '/images/results/cropped/result_5.png',

			theory: [
				'Farbtypen.',
				'Mathematischer und mechanischer Verlauf auf Latex.',
				'Farbauftrag-Dichte.',
				'Neigungswinkel der Maschine zur Haut.',
				'Hautspannung während der Arbeit.',
				'Primäre und sekundäre Anästhesie',
				'Modulfenster.',
				'Tyndall-Effekt.',
				'Effizientes Arbeiten und korrekte Pixel. Abhängigkeit des Pixels von Handgeschwindigkeit und Spannung.'
			],
			practice: [
				'Farbauftrag-Dichte.',
				'Neigungswinkel der Maschine zur Haut.',
				'Hautspannung während der Arbeit.',
				'Primäre und sekundäre Anästhesie',
				'Modulfenster.',
				'Tyndall-Effekt.',
				'Effizientes Arbeiten und korrekte Pixel. Abhängigkeit des Pixels von Handgeschwindigkeit und Spannung.'
			]
		},
		{
			title: 'Farbe und Pigmente im Permanent-Make-up',
			id: 5,
			avif: '/images/results/cropped/result_4-5.avif',
			link: '/', image: '/images/results/cropped/result_4-5.png',

			theory: [
				'Pigmente. Farbauswahl. Korrekturen.',
				'Bunte Schatten und warum nicht.',
				'Organische und mineralische Materialien bei der Arbeit mit Augen.'
			],
			practice: []
		},
		{
			title: 'Fotoinhalte und Teilnahme an Wettbewerben',
			id: 6,
			avif: '/images/results/cropped/result_6.avif',
			link: '/', image: '/images/results/cropped/result_6.png',

			theory: [
				'Retusche, Fotos und Software zur Bild- und Videobearbeitung.',
				'Teilnahme an Wettbewerben. Unterschied zwischen Modell und Kunde.',
				'Wie wählt man ein Modell für Wettbewerbe aus und bereitet es vor.'
			],
			practice: []
		},
		{
			title: 'Verfahrensregeln und Arbeitsabläufe',
			id: 7,
			avif: '/images/results/cropped/result_7.avif',
			link: '/', image: '/images/results/cropped/result_7.png',

			theory: [
				'Schattierungsrichtungsschema für die Arbeit mit den Augen. Arbeit mit beiden Augen.',
				'Latex. Übung der Schattierungsrichtungen.',
				'Wimpernlinie.',
				'Wimpernlinie, Demonstrationsarbeit.',
				'Schritt-für-Schritt Arbeitsregel.',
				'Schritt-für-Schritt Arbeitsregel auf Latex.'
			],
			practice: [
				'Latex. Übung der Schattierungsrichtungen.',
				'Wimpernlinie, Demonstrationsarbeit.',
				'Schritt-für-Schritt Arbeitsregel auf Latex.'
			]
		},
		{
			title: 'Demonstrationsarbeiten an Modellen. Fehleranalyse',
			id: 8,
			avif: '/images/results/cropped/result_8.avif',
			link: '/', image: '/images/results/cropped/result_8.png',

			theory: [
				'Demonstrationsarbeit am Modell. Gesamtplan.',
				'Korrektutheorie.',
				'Korrektur am Modell. Gesamtplan.',
				'Demonstrationsarbeit am Modell. Makro.',
				'Korrektur am Modell. Makro.',
				'Empfehlungen vor und nach der Prozedur.',
				'Analyse häufiger Fehler.'
			],
			practice: [
				'Demonstrationsarbeit am Modell. Gesamtplan.',
				'Korrektur am Modell. Gesamtplan.',
				'Demonstrationsarbeit am Modell. Makro.',
				'Korrektur am Modell. Makro.'
			]
		}
	]
};

export default de;
